...
function Dashboard() 
{
    ...
    var tokenToBeSent = window.sessionStorage.getItem("loginToken");
    ...
    const AddRecord = ()=>{
         axios.post(url, emp,{headers: 
                    {'authorization': 
                    "bearer " + tokenToBeSent}}).then((result)=>{
    ...
    const RemoveRecord = (No)=>{
        ...
        axios.delete(updatedUrl,{headers: 
                    {'authorization': 
                    "bearer " + tokenToBeSent}}).then((result)=>{
           if(result.data.affectedRows!==undefined &&
              result.data.affectedRows > 0)
    ...
    const UpdateRecord = ()=>
    {
        var updatedUrl = url + "/" + emp.No;
         ...
         axios.put(updatedUrl, emp,{headers: 
                    {'authorization': 
                    "bearer " + tokenToBeSent}}).then((result)=>{
           if(result.data.affectedRows!==undefined &&
              result.data.affectedRows > 0)
    ...
    return (<div className='container'>
                <center>
                <button style={{float: 'right'}} onClick={Logout} className='btn btn-primary'>Logout</button>
                 <UpperTable emp={emp} 
